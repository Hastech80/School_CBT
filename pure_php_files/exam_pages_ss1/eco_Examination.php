<?php   
        ...
        if (!isset($_SESSION['shuffled_questions'])) {
            $query = "SELECT * FROM economics_ss1 ORDER BY RAND()";
...
if (isset($_POST['Submit'])) {
    ...
    if (mysqli_num_rows($result1) > 0 ) {
        $query_update = "UPDATE result_ss1 SET econo = $student_score WHERE name = \"$student_name\"";
        mysqli_query($conn, $query_update);
    } else {
        $query_insert = "INSERT INTO result_ss1 (name, econo) VALUES ('$student_name','$student_score')";
        mysqli_query($conn, $query_insert);
    }
    
    header("location: ../sss1_finished/econo_finished.php");
}
if (!isset($_SESSION['timer'])) {
    $time_value = $conn->query("SELECT time FROM ss1_timer_table WHERE subjectt = 'Economics'")->fetch_column();
