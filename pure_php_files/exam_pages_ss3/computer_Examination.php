<?php   
        ...
        if (!isset($_SESSION['shuffled_questions'])) {
            $query = "SELECT * FROM computer_sci_ss3 ORDER BY RAND()";
...
if (isset($_POST['Submit'])) {
    ...
    if (mysqli_num_rows($result1) > 0 ) {
        $query_update = "UPDATE result_ss3 SET computer_sci = $student_score WHERE name = \"$student_name\"";
        mysqli_query($conn, $query_update);
    } else {
        $query_insert = "INSERT INTO result_ss3 (name, computer_sci) VALUES ('$student_name','$student_score')";
        mysqli_query($conn, $query_insert);
    }
    
    header("location: ../sss3_finished/computer_finished.php");
}
if (!isset($_SESSION['timer'])) {
    $time_value = $conn->query("SELECT time FROM ss3_timer_table WHERE subjectt = 'Computer_Sci.'")->fetch_column();
