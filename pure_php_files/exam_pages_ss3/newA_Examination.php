<?php   
        ...
        if (!isset($_SESSION['shuffled_questions'])) {
            $query = "SELECT * FROM new_A_ss3 ORDER BY RAND()";
...
if (isset($_POST['Submit'])) {
    ...
    if (mysqli_num_rows($result1) > 0 ) {
        $query_update = "UPDATE result_ss3 SET new_A = $student_score WHERE name = \"$student_name\"";
        mysqli_query($conn, $query_update);
    } else {
        $query_insert = "INSERT INTO result_ss3 (name, new_A) VALUES ('$student_name','$student_score')";
        mysqli_query($conn, $query_insert);
    }
    
    header("location: ../sss3_finished/newA_finished.php");
}
if (!isset($_SESSION['timer'])) {
    $time_value = $conn->query("SELECT time FROM ss3_timer_table WHERE subjectt = 'New_A'")->fetch_column();
