<?php   
        ...
        if (!isset($_SESSION['shuffled_questions'])) {
            $query = "SELECT * FROM animal_hus_ss3 ORDER BY RAND()";
...
if (isset($_POST['Submit'])) {
    ...
    if (mysqli_num_rows($result1) > 0 ) {
        $query_update = "UPDATE result_ss3 SET animal_hus = $student_score WHERE name = \"$student_name\"";
        mysqli_query($conn, $query_update);
    } else {
        $query_insert = "INSERT INTO result_ss3 (name, animal_hus) VALUES ('$student_name','$student_score')";
        mysqli_query($conn, $query_insert);
    }
    
    header("location: ../sss3_finished/animal_finished.php");
}
if (!isset($_SESSION['timer'])) {
    $time_value = $conn->query("SELECT time FROM ss3_timer_table WHERE subjectt = 'Animal_Hus.'")->fetch_column();
